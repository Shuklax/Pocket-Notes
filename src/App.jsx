...
function App() {
  ...
  }); // State for the list of headings
  const [selectedHeading, setSelectedHeading] = useState(null); //state for currently selected heading.
  const [textareaValue, setTextareaValue] = useState("");

  //Save the headings to local Storage
  useEffect(() => {
      ...
      prevList.map(
        (heading) =>
          heading.name === selectedHeading.name
            ? { ...heading, notes: [] } // Create a new heading object with empty notes
            : heading // Keep other headings as-is
      )
    );
  };
  ...
  const handleSubmit = () => {
    ...
    const formattedDate = date.format("D MMM YYYY"); //9 Mar 2023
    const formattedTime = date.format("h:mm A"); //10:10 AM
